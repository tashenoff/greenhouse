...
//const nunjucks = require('gulp-cache');
...
function fontsProcess() {
  return gulp
    .src(['src/fonts/**/*'])
    .pipe(gulp.dest(`./dist/assets/${appVersion}/fonts`));
}
...
function cssProcess() {
  ...
  return gulp
    ...
    .pipe(gulp.dest(`./dist/assets/${appVersion}/css`));
}
...
function libsJsProcess() {
  return gulp
    ...
    .pipe(babel())
    .pipe(uglify())
    .pipe(gulp.dest(`./dist/assets/${appVersion}/js`));
}
...
const getVersion = () => {
  console.log('Версия сборки: ', appVersion);
};
